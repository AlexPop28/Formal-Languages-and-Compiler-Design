...
let detect_token
  ...
  let get_match regex = (Re2.find_submatches_exn regex program).(1) |> Option.value_exn in
    ...
    (match does_match separators with
     ...
     | false ->
       (match does_match operators with
        ...
        | false ->
          (match Finite_automaton.get_longest_accepted_prefix_exn constants program with
           | Some constant -> constant |> Token.Constant |> Ok
           | None ->
             (match
                Finite_automaton.get_longest_accepted_prefix_exn identifiers program
              with
              | Some id -> id |> Token.Identifier |> Ok
              | None ->
                Or_error.error_string
                  [%string "Lexical error at line %{line_number#Int}: %{program}"]))))
;;
...
let scan_with_tokens_data ~constants ~identifiers ~(tokens_data : Tokens_data.t) ~program =
  let wrap_each_char =
    List.map ~f:(fun s ->
      let wrapped = ref "" in
